...
```java
f(arr1, arr2, k);// 返回两个有序数组合并且整体有序后的第k个数
```

如果我们能实现这个函数，那么当arr3为奇数时，中位数就是arr3[k]，为偶数时中位数就是（arr3[k] + arr3[k + 1])/2

再实现一个函数g

```java
g(arr1, arr2); // 当arr1和arr2长度一样的情况下，返回上中位数的值

// 等同于

g(arr1, L1, R1, arr2, L2, R2);// 其中R1 - L1 = R2 - L2
```
...
* arr1 和 arr2的上中位数相等时，整体的中位数就是这两个中位数之一
...
利用f函数求两个可能不等长的有序数组的中位数

f函数可以返回两个函数合并排序后第k小的数
